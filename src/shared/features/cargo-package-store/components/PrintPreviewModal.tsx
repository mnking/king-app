...
interface PrintPreviewModalProps {
  open: boolean;
  onClose: () => void;
  packingListDetail: PackingListPreview | null | undefined;
        ...
        <div className="grid gap-2 text-sm text-gray-700 dark:text-gray-200">
          ...
          <div className="grid grid-cols-2 gap-2 rounded-md bg-gray-50 p-3 dark:bg-gray-800">
            <div className="inline-flex items-center gap-2">
              <FileText className="h-4 w-4 text-indigo-500" /> Packing List: {packingListDetail.packingListNumber ?? packingListDetail.id}
            </div>
            <div className="inline-flex items-center gap-2">
              <Ship className="h-4 w-4 text-emerald-500" /> Forwarder: {packingListDetail.hblData?.forwarderName ?? '—'}
            </div>
            <div className="inline-flex items-center gap-2">
              <PackageIcon className="h-4 w-4 text-blue-500" /> HBL: {packingListDetail.hblData?.hblCode ?? '—'}
            </div>
            <div className="inline-flex items-center gap-2">
              <PackageIcon className="h-4 w-4 text-cyan-500" /> Container: {packingListDetail.hblData?.containerNumber ?? '—'}
            </div>
            <div className="inline-flex items-center gap-2">
              <PackageIcon className="h-4 w-4 text-indigo-400" /> Type: {packingListDetail.hblData?.containerType ?? '—'}
            </div>
            <div className="inline-flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-emerald-500" /> Seal: {packingListDetail.hblData?.sealNumber ?? '—'}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 rounded-md bg-gray-50 p-3 dark:bg-gray-800">
