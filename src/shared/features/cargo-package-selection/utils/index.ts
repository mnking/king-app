...
export const groupPackagesByLine = (
  ...
  packages.forEach((pkg) => {
    ...
    map.set(key, {
      key,
      lineNo: pkg.lineNo ?? null,
      lineId: pkg.lineId ?? null,
      cargoDescription: pkg.cargoDescription ?? pkg.packageNo ?? '',
  ...
  return Array.from(map.values()).sort((a, b) => {
    const aNo = a.lineNo ?? Number.MAX_SAFE_INTEGER;
    const bNo = b.lineNo ?? Number.MAX_SAFE_INTEGER;
    return aNo - bNo;
  });
};
