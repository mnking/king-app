...
import type { Forwarder, ForwarderCreateForm } from '../types';
import { zodResolver } from '@hookform/resolvers/zod';
import { ForwarderFormSchema, forwarderFormDefaultValues, forwarderTypeOptions } from '../schemas';
...
interface ForwarderModalProps {
  ...
  onClose: () => void;
  onSave: (payload: ForwarderCreateForm) => Promise<void>;
  isSaving?: boolean;
}

const emptyForm: ForwarderCreateForm = {
  ...forwarderFormDefaultValues,
};
...
export const ForwarderModal: React.FC<ForwarderModalProps> = ({
  open,
  mode,
  forwarder,
  onClose,
  ...
  const [contractFile, setContractFile] = useState<Forwarder['contractFile'] | null>(null);
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<ForwarderCreateForm>({
    defaultValues: emptyForm,
    resolver: zodResolver(ForwarderFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    if (forwarder && (mode === 'edit' || mode === 'view')) {
      const nextValues: ForwarderCreateForm = {
        code: forwarder.code || '',
        name: forwarder.name || '',
        status: forwarder.status || 'Active',
        type: forwarder.type || 'Forwarder',
        contactInfo: forwarder.contactInfo || '',
        contractStatus: forwarder.contractStatus || 'N/A',
        contractExpireDate: normalizeDateInput(forwarder.contractExpireDate),
        note: forwarder.note || '',
        contractFile: forwarder.contractFile ?? undefined,
      };
      reset(nextValues);
      setContractFile(forwarder.contractFile ?? null);
    } else {
      reset(emptyForm);
      setContractFile(null);
    }
  }, [forwarder, mode, reset]);
  ...
  const modalTitle = useMemo(() => {
    if (mode === 'edit') return 'Edit forwarder';
    if (mode === 'view') return 'Forwarder details';
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">
              Provide basic forwarder info and attach the contract file.
            </p>
          </div>
          <button
            type="button"
        ...
        <form
          ...
          onSubmit={handleSubmit(async (values) => {
            const payload: ForwarderCreateForm = {
              ...values,
              contractStatus:
                values.contractStatus && values.contractStatus !== 'N/A'
                  ? (values.contractStatus as Forwarder['contractStatus'])
                  : null,
        ...
        >
          <div className="space-y-5">
            <div className="rounded-xl border border-gray-200 bg-gray-50/80 p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/60">
              <div className="mb-3">
                <p className="text-sm font-semibold text-gray-900 dark:text-white">Forwarder information</p>
                <p className="text-xs text-gray-600 dark:text-gray-300">
                  Basic identifiers, status, and how to reach this forwarder.
                </p>
              </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800/60">
              ...
              <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-300">
                  Attach or download the contract PDF for this forwarder.
                </p>
                {contractFile ? (
