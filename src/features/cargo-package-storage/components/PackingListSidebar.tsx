...
import { storageDestuffingPlansQueryKey } from '../hooks/use-storage-destuffing-plans';
import { storageBookingOrdersByIdsQueryKey } from '../hooks/use-booking-orders-by-ids';
...
interface PackingListSidebarProps {
  ...
  className?: string;
  storedProgressByPl: Record<string, { storedCount: number; totalTarget: number | string }>;
...
export const PackingListSidebar: React.FC<PackingListSidebarProps> = ({
  packingLists,
  ...
  const handleApplyFilter = (values: FilterValues) => {
    ...
    queryClient.invalidateQueries({
      queryKey: ['cargo-packages-for-storage'],
    });
    queryClient.invalidateQueries({
      queryKey: storageBookingOrdersByIdsQueryKey,
    });
  };
  ...
  return (
    <div className={`h-full min-h-0 flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-900 ${className}`}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-3">
        {isPlanError ? (
          <div role="alert" className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-100">
        ...
        {showNoEligiblePlans ? (
          <div role="status" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900/50 dark:bg-amber-900/20 dark:text-amber-100">
        ...
        {visiblePackingLists.length > 0
          ? visiblePackingLists.map((pl) => {
              const cachedProgress = storedProgressByPl[pl.id];
              return (
                <PackingListCard
                  key={pl.id}
                  packingList={pl}
                  ...
                  // Pass cached progress here so the card shows the status
                  storedCount={cachedProgress?.storedCount}
