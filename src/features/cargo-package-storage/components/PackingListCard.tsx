    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
          <span className="line-clamp-2 font-medium break-words leading-tight" title={hblData?.shipper}>{hblData?.shipper ?? '—'}</span>
          <ArrowRight className="h-3 w-3 text-gray-400 flex-shrink-0 mt-0.5" />
          <span className="line-clamp-2 font-medium break-words leading-tight" title={hblData?.consignee}>{hblData?.consignee ?? '—'}</span>
        </div>

        {/* Details Grid */}
        <div className="grid grid-cols-1 gap-x-4 gap-y-1 text-sm text-gray-600 dark:text-gray-400">
