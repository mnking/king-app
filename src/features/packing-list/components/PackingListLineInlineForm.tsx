...
interface PackingListLineInlineFormProps {
  lineNumber: number;
...
export const PackingListLineInlineForm: React.FC<
  PackingListLineInlineFormProps
> = ({ line, onChange, readOnly = false, lineNumber }) => {
  const requiresImdg = (line.packageTypeCode ?? '').trim().toUpperCase() === 'DG';
  ...
  const handleSelectChange =
    (field: keyof PackingListLineFormValues) =>
      (event: React.ChangeEvent<HTMLSelectElement>) => {
        if (readOnly || !onChange) return;
        onChange(field, event.target.value as PackingListLineFormValues[typeof field]);
      };
    ...
    <div className="rounded-lg border border-blue-200 bg-blue-50/60 p-4 dark:border-blue-900/50 dark:bg-blue-900/20">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            <Label label="IMDG" required={requiresImdg} />
            <input
              value={line.imdg ?? ''}
              onChange={handleTextChange('imdg')}
              disabled={readOnly}
              placeholder="Enter IMDG code"
              className={inputClass}
            />
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-1">
          <div>
            <Label label="Shipmarks" required />
            <textarea
              rows={2}
...
export default PackingListLineInlineForm;
