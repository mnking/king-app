...
import { ImportWarehouseDeliveryModal } from './ImportWarehouseDeliveryModal';
import { useImportWarehouseDeliveryTab } from '../../hooks/useImportWarehouseDeliveryTab';

export const ImportWarehouseDeliveryTab = () => {
  const {
      ...
      <div className="flex flex-col gap-3 rounded-md border border-slate-200 p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
            ...
            Print Import Warehouse Delivery Note
          </div>
          <FormPrintButton
            templateCode="IMPORT_WAREHOUSE_DELIVERY_NOTE"
            context={context as any}
            overrides={manualFields as any}
            disabled={!context || isLoading || isFetching}
            onIssues={setIssueHints}
            fileName="import-warehouse-delivery-note.pdf"
            renderModal={({ open, issues: unresolvedIssues, onSubmit, onClose }) => (
              <ImportWarehouseDeliveryModal
                open={open}
